...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/config')
      ...
      .then((config) => {
        if (active) setGoogleEnabled(Boolean(config?.googleEnabled));
      })
      .catch(() => {
  ...
  const handleGoogleLogin = async () => {
    if (!googleEnabled) {
      showError('Google sign-in abhi configured nahi hai. Email se login ya signup karo.');
      return;
    }
    setIsLoading(true);
        ...
        <div className="space-y-6">
          <div className="relative group">
            <div className="relative p-[3px] rounded-full overflow-hidden shadow-lg transition-transform duration-200">
              ...
              <button id="google-login-btn" type="button" onClick={handleGoogleLogin} disabled={isLoading || !googleEnabled} className={"relative z-10 w-full h-14 px-6 rounded-full btn-3d-white flex items-center justify-center gap-3.5 font-semibold text-zinc-800 text-base " + (!googleEnabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer')}>
                <div className="w-6 h-6 flex items-center justify-center shrink-0"><GoogleIcon className="w-5 h-5" /></div>
                <span className="tracking-tight">{isLoading ? 'Connecting...' : googleEnabled ? 'Continue with Google' : 'Google sign-in unavailable'}</span>
              </button>
            </div>
          </div>
